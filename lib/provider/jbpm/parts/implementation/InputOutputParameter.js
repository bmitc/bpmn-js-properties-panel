...
function isScript(elem) {
  return is(elem, 'jbpm:Script');
}

function isList(elem) {
  return is(elem, 'jbpm:List');
}

function isMap(elem) {
  return is(elem, 'jbpm:Map');
}
...
module.exports = function(element, bpmnFactory, options) {
  ...
  entries.push(entryFactory.selectBox({
    ...
    set: function(element, values, node) {
      ...
      if (parameterType === 'script') {
        properties.definition = createParameterTypeElem('jbpm:Script');
      }
      else if (parameterType === 'list') {
        properties.definition = createParameterTypeElem('jbpm:List');
      }
      else if (parameterType === 'map') {
        properties.definition = createParameterTypeElem('jbpm:Map');
      }
  ...
  entries.push(entryFactory.table({
    ...
    addElement: function(element, node) {
      var bo = getSelected(element, node);
      var newValue = createElement('jbpm:Value', bo.definition, bpmnFactory, { value: undefined });
  ...
  entries.push(entryFactory.table({
    ...
    addElement: function(element, node) {
      var bo = getSelected(element, node);
      var newEntry = createElement('jbpm:Entry', bo.definition, bpmnFactory, { key: undefined, value: undefined });
