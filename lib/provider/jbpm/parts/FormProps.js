...
module.exports = function(group, element, bpmnFactory) {
  ...
  group.entries.push({
    ...
    html: '<div class="pp-row">' +
            '<label for="jbpm-form-type">Form Type</label>' +
            '<div class="pp-field-wrapper">' +
              '<select id="jbpm-form-type" name="formType" data-value>' +
    ...
    set: function(element, values, node) {
      ...
      if (values.formType === 'form-key') {
        ...
        var entry = getExtensionElements(bo, 'jbpm:FormData');
        ...
        commands.push(cmdHelper.updateBusinessObject(element, bo, { 'jbpm:formKey': undefined }));
        ...
        var formData = elementHelper.createElement('jbpm:FormData', { fields: [] }, extensionElements, bpmnFactory);
  ...
  group.entries.push(entryFactory.textField({
    ...
    set: function(element, values, node) {
      ...
      return cmdHelper.updateBusinessObject(element, bo, { 'jbpm:formKey': formKey });
    },
    disabled: function(element, node) {
  ...
  var formFieldsEntry = extensionElements(element, bpmnFactory, {
    ...
    createExtensionElement: function(element, extensionElements, value){
      var bo = getBusinessObject(element),
          formData = getExtensionElements(bo, 'jbpm:FormData')[0],
          field = elementHelper.createElement('jbpm:FormField', { id: value }, formData, bpmnFactory);
    ...
    removeExtensionElement: function(element, extensionElements, value, idx) {
      var formData = getExtensionElements(getBusinessObject(element), 'jbpm:FormData')[0],
  ...
  group.entries.push(entryFactory.comboBox({
    ...
    set: function(element, values, node) {
      ...
      if (selectedFormField.type === 'enum' && values.type !== 'enum') {
        // delete jbpm:value objects from formField.values when switching from type enum
  ...
  group.entries.push(entryFactory.table({
    ...
    addElement: function(element, node) {
      ...
      var enumValue = elementHelper.createElement('jbpm:Value', { id: id, name: undefined },
  ...
  group.entries.push(entryFactory.table({
    ...
    addElement: function(element, node) {
      ...
      if (!validation) {
        ...
        validation = elementHelper.createElement('jbpm:Validation', {}, getBusinessObject(element), bpmnFactory);

        commands.push(cmdHelper.updateBusinessObject(element, formField, { 'validation': validation }));
      }

      var newConstraint = elementHelper.createElement('jbpm:Constraint',
    ...
    removeElement: function(element, node, idx) {
      ...
      if (constraints.length === 1) {
        // remove jbpm:validation if the last existing constraint has been removed
  ...
  // [FormData] jbpm:properties table
